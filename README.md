...
> ⚠️ **安全边界须知**：CryptoWitch 是本地资料包的防护增强，**非 DRM**。三类敏感数据的保护强度不同：
> - **构建密码**：不进入二进制，文档内容受其保护（运行时由用户输入、Argon2id 派生密钥解密）。
> - **AI apiKey 与 endpoint**：以明文编译进 exe，**不受 vault 密码保护**，可被 `strings` 等静态分析工具从 exe 直接提取。请勿在 exe 中嵌入高价值、长期有效的 AI 凭证。
> - **MAC 白名单**：同样以明文落入二进制，非机密，仅为访问增强（可被伪造绕过）。
...
PDF 可能比普通 Markdown 大很多。分块加密和按块解密可避免在 Go 侧一次性解密整份 PDF，降低后端峰值内存压力；前端查看时仍会拼接为完整 PDF，内存占用与整份文件相当。
